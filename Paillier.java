...
public class Paillier {
    ...
    public BigInteger EncryptionNew(BigInteger m) {
        //(1+mn)r^n mod n^2
        BigInteger r = new BigInteger(bitLength, new Random());
        return (m.multiply(n).add(BigInteger.ONE)).multiply(r.modPow(n, nsquare));
    }
    ...
    // λ= lambda
    //  b=λ^–1 mod n
    //  m=bL(c λ mod n 2 ) mod n
    public BigInteger DecryptionNew(BigInteger c) {
        BigInteger u1 = c.modPow(lambda, nsquare);
        return lambda.modInverse(n).multiply(u1.subtract(BigInteger.ONE).divide(n)).mod(n);
    }
    ...
    public static void main(String[] str) {
        ...
        // 实例化两个数据对象m1,m2，进行加密
        BigInteger m1 = new BigInteger("10110");
        BigInteger m2 = new BigInteger("22111");
        ...
        BigInteger t1 = new BigInteger("2"); System.out.println("t1 = "+t1.toString());
        BigInteger t2 = new BigInteger("6"); System.out.println("t2 = "+t2.toString());
        ...
        System.out.println("改进可以实现字符串的加密算法之后");
        startTime=System.currentTimeMillis();
        String data = "鬼吹灯子秦岭迷窟";
        System.out.println("原内容为:"+data);
        //加密
        BigInteger encode1BI = encode2(data);
        em1 = paillier.Encryption(encode1BI);
        System.out.println("加密结果为:"+em1);
        // 解密
        BigInteger decode1BI = paillier.Decryption(em1);
        System.out.println("解密结果为:"+decode2(decode1BI));
        endTime=System.currentTimeMillis();
        System.out.println("运行时间："+(endTime-startTime)+"ns");


        System.out.println("改进可以实现字符串的加密算法优化之后");
        startTime=System.currentTimeMillis();
        String data1 = "鬼吹灯子精绝古城";
        System.out.println("原内容为:"+data1);
        //加密
        BigInteger m3 = encode2(data1);
        //加密
        BigInteger em3 = paillier.EncryptionNew(m3);
        //输出加密后的结果
        System.out.println("加密结果为: "+em3);
        BigInteger dm3 =  paillier.DecryptionNew(em3);
        //输出解密后的结果
        System.out.println("解密结果: "+decode2(dm3));
        endTime=System.currentTimeMillis();
        System.out.println("运行时间："+(endTime-startTime)+"ns");
    }
     ...
     * @return 返回解密后的新字符串
     */
    public static BigInteger encode2(String data) {
        byte[] b = data.getBytes();
        //遍历
        for(int i=0;i<b.length;i++) {
            if(b[i] < 0){
    ...
    public static String decode2(BigInteger bi) {
